...
pickerDateRange.prototype.checkDateRange = function(startYmd, endYmd) {
    ...
    console.log('1111111', startYmd, endYmd, sTime, eTime);
    if (eTime >= sTime) {
        // 顺序点击 开始时间小 结束时间大
        // 判断是否超过最大日期外
        ...
        // 点击的结束时间不能大于计算出来的结束时间
        if (eTime > maxEDate.getTime()) {
            // alert('结束日期不能大于：' + this.date2ymd(maxEDate).join('-'));
            alert('只能选择前后最多'+this.mOpts.dayRangeMax+'天');
            return false;
        }
    } else {
        // 逆序点击 开始时间大 结束时间小
        // 判断是否超过最大日期外
        minEDate = this.str2date(startYmd);
        minEDate.setMonth(minEDate.getMonth() - this.mOpts.monthRangeMax);
        minEDate.setDate(minEDate.getDate() - this.mOpts.dayRangeMax + 1);
        // 点击的结束时间不能小于计算出来的结束时间
        if (eTime < minEDate.getTime() ) {
            // alert('开始日期不能小于：' + this.date2ymd(minEDate).join('-'));
            alert('只能选择前后最多'+this.mOpts.dayRangeMax+'天');
            return false;
        }
    }
